...
> **Status: early but measured.** This repo is a fresh fork of the community-maintenance pattern from [`snailium/llama.cpp-sycl-intel-b70`](https://github.com/snailium/llama.cpp-sycl-intel-b70). The image builds and we now have **real B70 benchmarks** — including a per-depth long-context decode scan that isolates a backend-intrinsic Vulkan deep-context collapse (see below). 7900 XTX numbers are pending hardware arrival. See `STATUS.md` and `benchmark/` for what is verified vs. expected.

## 📊 Benchmarks (real B70 data)

- **[Backend test suite — one model, one fixed prompt battery → full backend profile](docs/PROMO-BACKEND-TEST-SUITE-20260910.md)** — the headline: t1–t5 (long-text + 3 real **agentic** tool-call tasks) + V1–V3 (vision), run with model/prompts/sampling held constant so swapping the backend (SYCL vs Vulkan) isolates backend-intrinsic performance. Reports both **speed** (TTFT / prefill / decode t/s / draft-acc) and **correctness**. This is the "honest single-session baseline" that datacenter agentic-serving benchmarks (MLPerf Agentic Inference, SemiAnalysis AgentX, Applied Compute) can't show — they measure serving clusters whose batching masks per-session KV-growth cost.
- **[Long-context agentic decode — per-depth scan](benchmark/results/2026-09-10-long-context-agentic-depth-scan.md)** — the deep-dive: Vulkan **decode collapses ~8x from 1k→64k context** (35.6 → 4.5 t/s) while SYCL degrades only ~2.4x; f16 KV collapses identically (not a KV-quant effect); **RDNA3 (RX 7900 XTX) does not collapse**. Methodology + raw JSON in `benchmark/`.
- **[Vulkan vs SYCL research writeup](docs/VULKAN-PERF-RESEARCH-20260910.md)** — root-cause analysis (scalar FA path at `n_rows==1`), MTP addendum, and re-test recipe.
