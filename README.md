...
- **[Long-context agentic decode — per-depth scan](benchmark/results/2026-09-10-long-context-agentic-depth-scan.md)** — the headline result: on a single Arc Pro B70, Vulkan **decode collapses ~8x from 1k→64k context** (35.6 → 4.5 t/s) while SYCL degrades only ~2.4x; f16 KV collapses identically (so it's not a KV-quant effect); and **RDNA3 (RX 7900 XTX) does not show the collapse**. This is the "honest single-session baseline" that datacenter agentic-serving benchmarks (MLPerf Agentic Inference, SemiAnalysis AgentX) can't show because their batching engines mask per-session KV-growth cost. Methodology + raw JSON in `benchmark/`.
